...
collision_check_job* make_collision_check_jobs(
  ...
  for (int32_t z = min_zoom; z <= max_zoom; z++) {
    ...
    const int32_t w = bot_right.x - top_left.x;
    const int32_t h = bot_right.y - top_left.y;
    ...
    for (int32_t i = 0; i < num_jobs - 1; i++) {
      ...
      collision_check_job job = {
          ...
          .bot_right = bot_right,
          .tile_size = tile_size,
          .x_start = top_left.x + (start_index % w),
          .y_start = top_left.y + (start_index / w),
          .x_end = top_left.x + (end_index % w),
          .y_end = top_left.y + (end_index / w),
          .zoom = z,
          .num_tile_coordinates = num_poly_tiles,
          .tile_coordinates = (vec2i*)calloc(num_poly_tiles, sizeof(vec2i))};

      memcpy(job.tile_coordinates, tile_coords, num_poly_tiles * sizeof(vec2i));
    ...
    collision_check_job last = {
        ...
        .bot_right = bot_right,
        .tile_size = tile_size,
        .x_start = top_left.x + last_job_idx % w,
        .y_start = top_left.y + last_job_idx / w,
        .x_end = bot_right.x,
        .y_end = bot_right.y,
        .zoom = z,
        .num_tile_coordinates = num_poly_tiles,
